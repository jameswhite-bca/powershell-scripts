...
<#
.SYNOPSIS
New-OrgVDCAllocation creates a new Virtual Data Centre in an existing vCloud organisation on the Pulsant Enterprise Cloud Platform (PEC).
.DESCRIPTION 
New-OrgVDCAllocation uses VMware PowerCli with the vCloud cmdlets to create a new Virtual Data center in an existing vCloud organisation on the Pulsant Enterprise Cloud Platform (PEC). The Allocation Model is set to Allocation pool.
...
.PARAMETER dr
Has the customer ordered DR with the solution? If they have then a Zerto vApp will be created
.EXAMPLE
.\New-OrgVDCAllocation.ps1 -accountcode PROV -vdcname 'PROV mkn1clouc2 VDC1' -cpu 5 -ram 4 -storagegb 100 -storageprofile cloumkn1c2corev2 -pecplatform mkn1clouc2 -dr True
#>
#>
[CmdletBinding()]
param (
...
# Remove the "* (any)" profile from the Org vDC completely  
...
# Changing VDC memory guaranteed percent, enabling thin provisioning and setting maximum networks to 3
Write-Host -ForegroundColor Yellow "Changing settings according to Deployment Standards"
get-orgvdc -Name $vdcname | Set-OrgVdc -MemoryGuaranteedPercent 90 -ThinProvisioned $true -NetworkMaxCount 3
