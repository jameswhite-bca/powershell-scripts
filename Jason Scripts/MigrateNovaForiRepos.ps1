...
if ($perform_bca_repo_overwrite) {
	...
	foreach ($repo in $repos) {
		...
		$push_url = $target_org + $target_project + "/_git/" + $direpo_name

		write-host "$repo_name, $repo_path ($repo_count of $repos_count)"

		$duplicate_repo = az repos show --repository $repo_name --org $target_org --project "$target_project" --output JSON | ConvertFrom-Json

		If (!($duplicate_repo)){
			$duplicate_repo = az repos create --name $repo_name --org $target_org --project "$target_project"  --output JSON | ConvertFrom-Json
		} else {
			az repos delete --id $duplicate_repo.id --org $target_org --project "$target_project" --yes
			$duplicate_repo = az repos create --name $repo_name --org $target_org --project "$target_project"  --output JSON | ConvertFrom-Json
		}
